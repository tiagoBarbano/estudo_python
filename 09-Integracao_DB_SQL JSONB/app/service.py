...
@router.get("/", status_code=status.HTTP_200_OK, response_model=list[ApoliceSchema])
...
async def get_user_data(id: str, db: AsyncSession = Depends(get_db)):
    user = await get_user_by_id_product(db, id)
